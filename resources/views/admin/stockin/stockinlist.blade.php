                    ...
                    <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        @foreach($result as $data)
                            <tr>
                                ...
                                <td>{{ $data->note }}</td>
                                <td>
                                {!! QrCode::size(100)->generate(url('viewstock/'.$data->id)); !!}
                        ...
                        ->generate('$data->uniqtag')) !!} "> --></td>
                                <td class="text-center"><a href="{{ url('viewstock/'.$data->id) }}" class="btn btn-primary">View</a><a href="{{ url('updatestockin/'.$data->id) }}" class="btn btn-primary">Edit</a> <a href="{{ url('deletestockin/'.$data->id) }}" class="btn btn-danger" onclick="if (!confirm('Are you sure to delete this item?')) { return false }">Delete</a> </td>
                                <?php $count += 1; ?>
                            </tr>
                        @endforeach
                        </tbody>
